...
extension ShadowPlugin {
    ...
    public func handleSystemAudioAndMicRecordingWithConfig(args: [String: Any]?, result: @escaping FlutterResult) {
        ...
        ShadowLogger.shared.info("MIC Config - \(micAudioConfig), SYS Config - \(systemAudioConfig)")
    ...
    public func handleStopSystemAudioAndMicRecording(result: @escaping FlutterResult) {
        ...
        Task {
            do {
                ...
                result("스크린 녹화 중지")
                ShadowLogger.shared.info("handleStopSystemAudioAndMicRecording called")
            } catch {
                handleError(error: error, result: result)
                ShadowLogger.shared.error("Stop Sys + Mic Recording Handler - \(error.localizedDescription)")
            }
        }
